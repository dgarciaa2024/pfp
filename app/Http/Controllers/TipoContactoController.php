...
class TipoContactoController extends Controller
{
    ...
    {
        $response = Http::get(env('API_URL', 'http://localhost:3002').'/get_tipo_contacto');
  ...
  if ($permisos) {
    ...
    {
        $response = Http::post(env('API_URL', 'http://localhost:3002').'/insert_tipo_contacto', [
    ...
    {
        $response = Http::put(env('API_URL', 'http://localhost:3002').'/update_tipo_contacto', [
